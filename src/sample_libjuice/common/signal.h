...
/* Socket type: int on Unix, SOCKET (handle) on Windows. Use int for simplicity. */
...
typedef struct p2p_signal_ctx {
	signal_sock_t sock;
	void *mutex;  /* pthread_mutex_t *, optional */
} p2p_signal_ctx_t;

/* Connect to signaling server. Returns 0 on success, -1 on error. */
int p2p_signal_connect(p2p_signal_ctx_t *ctx, const char *host, const char *port);

/* Send a JSON message: type and optional sdp. Returns 0 on success. */
int p2p_signal_send(p2p_signal_ctx_t *ctx, const char *type, const char *sdp);

/* Parse message type and optional sdp from JSON line. Returns 0 on success. */
int p2p_signal_parse_message(const char *json, char *type_out, size_t type_size,
                             char *sdp_out, size_t sdp_size);

/* Parse role from JSON (e.g. {"type":"role","role":"peer1"}). */
